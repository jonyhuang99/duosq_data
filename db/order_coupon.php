<?php
//优惠券使用加钱订单表操作基类，***订单相关操作出错必须throw Exception***
//子订单模块必须定义确认状态STATUS_PASS[到账网站]常量
namespace DB;

class OrderCoupon extends _Db {
	...
	function add($o_id, $user_id, $data=array()){
		...
		if(!$o_id || !$user_id || !$data['coupon_id']){
			throw new \Exception("[order_coupon][o_id:{$o_id}][add][param error]");
		}
		...
		$ret = parent::add($data);
		if(!$ret){
			throw new \Exception("[order_coupon][o_id:{$o_id}][add][save error]");
		}
		return $ret;
	}
	...
	function update($o_id, $new_field, $force=false){
		...
		if(!$o_id || !$new_field){
			throw new \Exception("[order_coupon][o_id:{$o_id}][update][param error]");
		}

		$old_detail = $this->find(array('o_id'=>$o_id));

		if(!$old_detail){
			throw new \Exception("[order_coupon][o_id:{$o_id}][update][o_id not exist]");
		}

		$ret = parent::update($o_id, arrayClean($new_field));

		if(!$ret){
			throw new \Exception("[order_coupon][o_id:{$o_id}][update][save error]");
		}
	...
	function afterUpdateStatus($o_id, $from, $to){
		...
		if(!$m_order){
			throw new \Exception("[order_coupon][afterUpdateStatus][m_order:{$o_id} not found]");
		}
		...
		if($from == self::STATUS_WAIT_CONFIRM && $to == self::STATUS_PASS){
			...
			if(!$ret){
				throw new \Exception("[order_coupon][o_id:{$o_id}][afterUpdateStatus][adjustBalanceForOrder error]");
			}
