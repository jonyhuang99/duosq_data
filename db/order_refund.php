<?php
//退款订单表操作基类，***订单相关操作出错必须throw Exception***
//子订单模块必须定义确认状态STATUS_PASS[到账网站]常量
namespace DB;

class OrderRefund extends _Db {
	...
	const STATUS_INVALID = 10; //退款无效

	/**
	 * 新增用户退款订单
	 * @param char    $o_id     主订单编号
	 * @param bigint  $user_id  用户ID
	 * @param array   $data     订单初始数据，退款原订单号refer_o_id
	 * return char              主订单编号
	 */
	function add($o_id, $user_id, $data=array()){

		if(!$o_id || !$user_id || !$data['refer_o_id']){
			throw new \Exception("[order_refund][o_id:{$o_id}][add][param error]");
		}
		...
		if(!$ret){
			throw new \Exception("[order_refund][o_id:{$o_id}][add][save error]");
		}
		return $ret;
	}
	...
	function update($o_id, $new_field){
		...
		if(!$o_id || !$new_field){
			throw new \Exception("[order_refund][o_id:{$o_id}][update][param error]");
		}

		$old_detail = $this->find(array('o_id'=>$o_id));

		if(!$old_detail){
			throw new \Exception("[order_refund][o_id:{$o_id}][update][o_id not exist]");
		}
		...
		if(!$ret){
			throw new \Exception("[order_refund][o_id:{$o_id}][update][save error]");
		}
	...
	function afterUpdateStatus($o_id, $from, $to){
		...
		if(!$m_order){
			throw new \Exception("[order_refund][afterUpdateStatus][m_order:{$o_id} not found]");
		}

		//退款订单状态由 待处理 => 已通过，进行账号加款
		if($from == self::STATUS_WAIT_CONFIRM && $to == self::STATUS_PASS){
			//调整资产
			$ret = D('fund')->adjustBalanceForOrder($o_id);
			if(!$ret){
				throw new \Exception("[order_refund][o_id:{$o_id}][afterUpdateStatus][adjustBalanceForOrder error]");
			}
		}

		//退款订单状态 => 不通过，进行账号扣除流水，主订单变为不通过
		if($to == self::STATUS_INVALID){

			$errcode = '';
			$ret = D('fund')->reduceBalanceForOrder($o_id, $errcode);
			if(!$ret){
				throw new \Exception("[order_refund][o_id:{$o_id}][afterUpdateStatus][reduceBalance error]["._e($errcode, false)."]");
			}
